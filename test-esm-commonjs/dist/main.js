...
class VFile {
  ...
  set path(path) {
    if (isUrl(path)) {
      path = require$$2.fileURLToPath(path);
    }
 ...
 * @param {Image} node
 */
function image$1(node, _, context, safeOptions) {
  const quote = checkQuote(context);
  const suffix = quote === '"' ? 'Quote' : 'Apostrophe';
 ...
 */
function paragraph$1(node, _, context, safeOptions) {
...
var fetch = globalThis.fetch ?? async function undiciFetchWrapper(...args) {
  if (!_undici) {
    _undici = await Promise.resolve().then(function () { return require('./index-1861421a.js'); }).then(function (n) { return n.index; });
  }
...
var ChatGPTAPI = class {
  constructor(opts) {
    ...
    this._accessTokenCache = new ExpiryMap$1(accessTokenTTL);
    if (accessToken) {
